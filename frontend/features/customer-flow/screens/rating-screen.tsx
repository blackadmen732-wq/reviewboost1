        ...
        {rating ? (
          <div className="rating-note" data-rating-note>
            <label className="field-label" htmlFor="private-note">
              {messages.noteLabel} <span className="field-optional">({messages.optional})</span>
            </label>
            <Textarea
              id="private-note"
              name="private-note"
              ...
              disabled={isSubmitting}
              aria-describedby={noticeId ? `note-trust ${noticeId}` : "note-trust"}
            />
            ...
            {hasError ? (
              <InlineNotice id="feedback-status" kind="error" title={messages.submitErrorHeading} blocking>
