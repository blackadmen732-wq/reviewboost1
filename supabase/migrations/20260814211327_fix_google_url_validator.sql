-- Rewrite _validate_google_review_url to safely handle all edge cases.
--
-- The previous version crashed on host-only URLs (no trailing slash),
-- mishandled credentials with ports, and could throw on malformed input.
-- This version matches the TypeScript isValidGoogleReviewUrl behavior:
...
begin
    ...
    if v_colon_pos > 0 then
        v_host := lower(substring(v_authority from 1 for v_colon_pos - 1));
    else
